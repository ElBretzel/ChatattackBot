...
class PaginatorBuilder(AbstractPaginatorBuilder):
    def __init__(self):
        ...
        self.decorator = ""
        self._content = []
        self.max_content = 20  # * Max element in one page (not characters)
    ...
    def paginator_store(self) -> None:
        ...
        """
        assert isinstance(self.base_embed, discord.Embed), "Error of the base embed is not created."
        assert self._content, "Error if there is no content in the paginator."
    ...
    def content_builder(self, **kwargs: object) -> None:
        ...
        **format : str, optionnal
        ...
        if self.separator == "\n" or self.prefix in ["/number/", "/emote/"]:  # * To avoid long embed
    ...
    def _content_arrangement(self) -> list:
        ...
        for i, content in enumerate(self._embed_content):
            ...
            if n_char_cache >= 1800 or (i + 1) % self.max_content == 0:  # * Check if a exceed max character in a message or if there is x content in the page
...
class PaginatorController:
    ...
    async def paginator_emote(self) -> str:
        ...
        Notes
        ----------
        If the user close the paginator, return a None object.
    ...
    async def _paginator_wait_for(self, **kwargs) -> tuple:
        ...
        if kwargs.get("type") == "message":
            ...
            pending_tasks = [self.client.wait_for('message', timeout=60, check=check_message),
                             self.client.wait_for('reaction_add', timeout=60, check=check_reaction)]
            ...
            pending_tasks = [self.client.wait_for('reaction_add', timeout=60, check=check_reaction)]
